...
interface CreateCharterDriverData {
  fullname: string;
  email: string;
  phone: string;
  password: string;
...
const CharterAddDriver: React.FC = () => {
  ...
  const onFinish = async (values: CreateCharterDriverData) => {
    setLoading(true);
    try {
      ...
      if (response.user && response.user.id) {
        ...
        setStep(1);
        message.info('Now you can add charter driver details like license, qualifications, and address.');
      }
      
    } catch (error: any) {
      console.error('Create charter driver error:', error);
      message.error(error.message || 'Failed to create charter driver account');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAddDriverDetails = () => {
    if (createdDriverId) {
      navigate(`/admin/charter/drivers/${createdDriverId}/add-details`);
    }
  };

  const handleSkipForNow = () => {
    message.success('Charter driver account created. You can add details later.');
        ...
        <div className="text-center py-8">
          <div className="mb-6">
            ...
            <Text className="text-gray-600">
              Charter driver account has been created. Now you can add driver details like license number, qualifications, and address.
            </Text>
          </div>
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white p-4">
      <div className="max-w-4xl mx-auto">
        <div className="mb-8">
          ...
          <Title level={2} className="text-gray-800">Add New Charter Driver</Title>
          <Text className="text-gray-600">Create a new charter driver account with basic information</Text>
        </div>
...
export default CharterAddDriver;